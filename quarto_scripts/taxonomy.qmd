...
FastANI output files are stored in `04_fastani` directory in the specified `outdir`.

Following is the command used for ANI calculation post assembly:

``` bash
fastANI -q {sample_dir/02_unicycler/assembly.fasta} -r {reference.fasta} -o {sample_dir/04_fastANI/04_fastANI_post_assembly.txt} -t 20 &> {sample_dir/logs/fastani_post_assembly.log}
```

Following is the command used for ANI calculation post polishing:

``` bash
fastANI -q {sample_dir/08_pilon/{sample_name}.fasta} -r {reference.fasta} -o {sample_dir/04_fastANI/04_fastANI_post_polish.txt} -t 20 &> {sample_dir/logs/fastani_post_polish.log}
```

```{r taxonomy, echo=FALSE}
paths <- readLines(file_path)
ANI_post_assembly <- paths[8]
ANI_post_polish <- paths[9]
...
|-------------------------------------|------------------|------------------|
| [04_fastANI_post_assembly.txt](file:// `r file.path(ANI_post_assembly)`) | sample_dir/04_fastANI | ANI calculation after assembly  |
| [04_fastANI_post_polish.txt](file:// `r file.path(ANI_post_polish)`)     | sample_dir/04_fastANI | ANI calculation after polishing |
...
AAI output files are stored in `05_ezaai` directory in the specified `outdir`.

Following is the command used for AAI calculation:

``` bash
mkdir -p {sample_dir/05_ezaai/tmp/}
ezaai extract -i {sample_dir/08_pilon/{sample_name}.fasta} -o {sample_dir/05_ezaai/tmp/}{sample_name}_ezaai.db &> {sample_dir/logs/ezaai.log}
ezaai extract -i {reference.fasta} -o {sample_dir/05_ezaai/tmp/}ref_ezaai.db &> {sample_dir/logs/ezaai.log}
ezaai calculate -i {sample_dir/05_ezaai/tmp/}/{sample_name}_ezaai.db -j {sample_dir/05_ezaai/tmp/}/ref_ezaai.db -o {sample_dir/05_ezaai/{sample_name}_aai} &> {sample_dir/logs/ezaai.log}
mv {sample_dir/05_ezaai/{sample_name}_aai} {sample_dir/05_ezaai/{sample_name}_aai.tsv}
rm -r {sample_dir/05_ezaai/tmp/}
```
...
| Filename                                      | Location            | Description                     |
|-------------------------------|-------------------|-----------------------|
| [aai.tsv](file:// `r file.path(AAI)`) | sample_dir/05_ezaai | AAI calculation after polishing |
...
POCP output files are stored in `05_pocp` directory in the specified `outdir`.
...
| Filename                                               | Location           | Description                             |
|-------------------------------|-------------------|-----------------------|
| [pocp-matrix.tsv](file:// `r file.path(POCP)`) | sample_dir/05_pocp | POCP calculation matrix after polishing |
...
Barrnap output files are stored in `11_barrnap` directory in the specified `outdir`.

Following is the command used for extracting rRNA sequences:

``` bash
barrnap -o {sample_dir/11_barrnap/{sample_name}_rrna.fa} <{sample_dir/08_pilon/{sample_name}.fasta} --threads 20 &> {sample_dir/logs/barrnap.log}
```
...
| Filename                                          | Location              | Description                        |
|-------------------------------|-------------------|-----------------------|
| [rrna.fa](file:// `r file.path(barrnap)`) | sample_dir/11_barrnap | Fasta file with hit rRNA sequences |
