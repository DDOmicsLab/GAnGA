...
outdir <- paths[37] 
sample_dir <- paths[39]
post_assembly_quast_dir <- paths[27]
post_polish_quast_dir <- paths[28]
post_assembly_checkm_dir <- paths[29]
post_polish_checkm_dir <- paths[30]
...
unzip(fastqc_zip_files[1], exdir = file.path(qc.dir, "unzipped_forward"))
qc_data_forward <- (file.path(qc.dir, "unzipped_forward", "forward_paired_fastqc")) 
...
unzip(fastqc_zip_files[2], exdir = file.path(qc.dir, "unzipped_reverse"))
qc_data_reverse <- (file.path(qc.dir, "unzipped_reverse", "reverse_paired_fastqc")) 
...
|------------------------------|-------------------|-----------------------|
| [pretrim_forward_paired_fastqc.html](file://`r file.path(pretrim_qc.dir, paste0(unzipped_dir_forward, '.html'))`) | sample_dir/01_fastqc               | Quality check of forward paired reads before trimming |
| [pretrim_reverse_paired_fastqc.html](file://`r file.path(pretrim_qc.dir, paste0(unzipped_dir_reverse, '.html'))`) | sample_dir/01_fastqc               | Quality check of reverse paired reads before trimming |
| [posttrim_forward_paired_fastqc.html](file://`r file.path(qc.dir,'forward_paired_fastqc.html')`)                      | sample_dir/00_trimmed_reads/Fastqc | Quality check of forward paired reads after trimming  |
| [posttrim_reverse_paired_fastqc.html](file://`r file.path(qc.dir,'reverse_paired_fastqc.html')`)                      | sample_dir/00_trimmed_reads/Fastqc | Quality check of reverse paired reads after trimming  |
...
CheckM output files are stored in `03_checkm/post_assembly` directory in the specified `outdir`.

Following is the command used for assembling the genome:

``` bash
checkm lineage_wf {sample_dir/02_unicycler/} {sample_dir/03_checkm/post_assembly/} -t 20 &> {sample_dir/03_checkm/post_assembly/table.txt}
sed -i '1,34d' {sample_dir/03_checkm/post_assembly/table.txt}
checkm ssu_finder {sample_dir/02_unicycler/assembly.fasta} -x fasta {sample_dir/02_unicycler/} {sample_dir/03_checkm/post_assembly/} -t 20 &> {sample_dir/logs/checkm_post_assembly.log}
```
...
| Filename                                                                                     | Location                           | Description                                                  |
|------------------------------|-------------------|-----------------------|
| [table.txt](file://`r file.path(checkm_post)`)                                         | sample_dir/03_checkm/post_assembly | Completeness and contamination statistics after assembly     |
| [ssu.fna](file://`r file.path(post_assembly_checkm_dir, 'ssu.fna')`)                 | sample_dir/03_checkm/post_assembly | Identification of SSU (16S) rRNA in sequences after assembly |
| [ssu_summary.tsv](file://`r file.path(post_assembly_checkm_dir, 'ssu_summary.tsv')`) | sample_dir/03_checkm/post_assembly | Summary statistics of SSU rRNAs                              |
...
CheckM output files are stored in `03_checkm/post_polish` directory in the specified `outdir`.

Following is the command used for assembling the genome:

``` bash
checkm lineage_wf {sample_dir/08_pilon/} {sample_dir/03_checkm/post_polish/} -t 20 &> {sample_dir/03_checkm/post_polish/table.txt}
sed -i '1,34d' {sample_dir/03_checkm/post_polish/table.txt}
checkm ssu_finder {sample_dir/08_pilon/{sample_name}.fasta} -x fasta {sample_dir/08_pilon/} {sample_dir/03_checkm/post_polish/} -t 20 &> {sample_dir/logs/checkm_post_polish.log}
```
...
| Filename                                                                                   | Location                         | Description                                                   |
|------------------------------|-------------------|-----------------------|
| [table.txt](file://`r file.path(checkm_post)`)                                       | sample_dir/03_checkm/post_polish | Completeness and contamination statistics after polishing     |
| [ssu.fna](file://`r file.path(post_polish_checkm_dir, 'ssu.fna')`)                 | sample_dir/03_checkm/post_polish | Identification of SSU (16S) rRNA in sequences after polishing |
| [ssu_summary.tsv](file://`r file.path(post_polish_checkm_dir, 'ssu_summary.tsv')`) | sample_dir/03_checkm/post_polish | Summary statistics of SSU rRNAs                               |
...
QUAST output files are stored in `03_quast/post_assembly` directory in the specified `outdir`.

Following is the command used for assembling the genome:

``` bash
quast.py -o {sample_dir/03_quast/post_assembly} -r {reference.fasta} --circos {sample_dir/03_quast/assembly.fasta} 
```
...
| Filename                                                                         | Location                          | Description                                                                                |
|------------------------------|-------------------|-----------------------|
| [report.html](file://`r file.path(post_assembly_quast_dir,'report.html')`) | sample_dir/03_quast/post_assembly | Quality check after assembly with HTML version of the report with interactive plots inside |
| [icarus.html](file://`r file.path(post_assembly_quast_dir,'icarus.html')`) | sample_dir/03_quast/post_assembly | Icarus main menu with links to interactive viewers                                         |

### Draft genome QC- Post Polishing

This is a brief statistics table of quality check of draft genome (generated after polishing) done using [QUAST](https://github.com/ablab/quast?tab=readme-ov-file).

QUAST output files are stored in `03_quast/post_polish` directory in the specified `outdir`.

Following is the command used for assembling the genome:

``` bash
quast.py -o {sample_dir/03_quast/post_polish} -r {reference.fasta} --circos {sample_dir/08_pilon/{sample_name}.fasta} 
```
...
| Filename                                                                       | Location                        | Description                                                                                 |
|------------------------------|-------------------|-----------------------|
| [report.html](file://`r file.path(post_polish_quast_dir,'report.html')`) | sample_dir/03_quast/post_polish | Quality check after polishing with HTML version of the report with interactive plots inside |
| [icarus.html](file://`r file.path(post_polish_quast_dir,'icarus.html')`) | sample_dir/03_quast/post_polish | Icarus main menu with links to interactive viewers                                          |
:::
