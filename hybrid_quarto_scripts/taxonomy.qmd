...
FastANI output files are stored in `06_fastani` directory in the specified `outdir`.

Following is the command used for ANI calculation post assembly:

``` bash
fastANI -q {sample_dir/02_flye/assembly.fasta} -r {reference.fasta} -o {sample_dir/06_fastani/06_fastani_post_assembly.txt} -t 20 &> {sample_dir/logs/fastani_post_assembly.log}
```

Following is the command used for ANI calculation post polishing:

``` bash
fastANI -q {sample_dir/04_pilon/polish.fasta -r {reference.fasta} -o {sample_dir/06_fastani/06_fastani_post_polish.txt} -t 20 &> {sample_dir/logs/fastani_post_polish.log}
```

```{r taxonomy, echo=FALSE}
paths <- readLines(file_path)
ANI_post_assembly <- paths[9]
ANI_post_polish <- paths[10]
...
|-------------------------------------|------------------|------------------|
| [06_fastani_post_assembly.txt](file:// `r file.path(ANI_post_assembly)`) | sample_dir/06_fastani | ANI calculation after assembly  |
| [06_fastani_post_polish.txt](file:// `r file.path(ANI_post_polish)`)     | sample_dir/06_fastani | ANI calculation after polishing |
...
| Filename                                      | Location            | Description                     |
|-------------------------------|-------------------|-----------------------|
| [aai.tsv](file:// `r file.path(AAI)`) | sample_dir/07_ezaai | AAI calculation after polishing |
...
| Filename                                               | Location           | Description                             |
|-------------------------------|-------------------|-----------------------|
| [pocp-matrix.tsv](file:// `r file.path(POCP)`) | sample_dir/07_pocp | POCP calculation matrix after polishing |
...
| Filename                                          | Location              | Description                        |
|-------------------------------|-------------------|-----------------------|
| [rrna.fa](file:// `r file.path(barrnap)`) | sample_dir/10_barrnap | Fasta file with hit rRNA sequences |
